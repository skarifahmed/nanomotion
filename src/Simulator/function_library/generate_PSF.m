...
function [I_map,G,I_out]=generate_PSF (Sensor,vec_r_foc,vec_r_ccd)

NA=Sensor.NA;
n_obj_n_ccd=Sensor.n_obj_n_ccd;
f_obj_f_ccd=Sensor.f_obj_f_ccd;
lam_um=Sensor.wavelength;
...
th_obj=linspace(0,theta_max_obj,100);
...
% if ~(size(vec_r_foc,1)==1 && vec_r_foc(1)==0)
    vec_r_foc=[0.0 0 0];
[I_map,G,~]=generate_PSF (Sensor,vec_r_foc,vec_r_ccd*1e6);
end